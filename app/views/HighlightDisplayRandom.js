...
export default {
  ...
  setup() {
    ...
    const fetchRandomHighlight = async () => {
      try {
        const response = await api.getRandomHighlight();
        if (response.success) {
          highlight.value = response.data.highlight;
        } else {
          error.value = 'No highlights available';
        }
      } catch (err) {
        error.value = err.message || 'Failed to load highlight';
        console.error('Error loading random highlight:', err);
      } finally {
        loading.value = false;
      }
    };
  ...
  template: `
    <div class="h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-8 flex flex-col justify-center relative">
      <!-- Loading State -->
      ...
      <div v-else-if="highlight" class="max-w-4xl mx-auto w-full">
        ...
        <!-- Metadata Footer -->
        <div class="mt-12 pt-8 border-t border-yellow-400/20 flex gap-3 flex-wrap justify-between items-center">
          ...
          <button
            @click="loadNew"
            :disabled="loading"
            class="px-6 py-2 bg-yellow-400/20 hover:bg-yellow-400/30 border border-yellow-400/50 rounded-lg text-yellow-300 font-medium transition disabled:opacity-50"
          >
            {{ loading ? 'Loading...' : 'Load Another' }}
          </button>
        </div>
      </div>
