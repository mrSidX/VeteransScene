    ...
    <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-gray-900 border-2 border-gray-700 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-6 space-y-6">
          <!-- Name -->
          <div>
            <label class="block text-sm font-semibold text-gray-300 mb-2">List Name *</label>
            <input
              v-model="form.name"
              type="text"
          ...
          <!-- List Type -->
          <div>
            <label class="block text-sm font-semibold text-gray-300 mb-3">List Type *</label>
            <div class="space-y-3">
          ...
          <!-- Dynamic List: Roles -->
          <div v-if="form.isDynamic" class="space-y-3 p-4 bg-gray-800/50 border border-gray-700 rounded-lg">
            ...
            <label class="flex items-center cursor-pointer">
              ...
              <span class="ml-2 text-sm text-gray-300">Include only active users</span>
            </label>
          </div>

          <!-- Static List: Recipients -->
          <div v-else class="space-y-3 p-4 bg-gray-800/50 border border-gray-700 rounded-lg">
            <label class="block text-sm font-semibold text-gray-300 mb-2">Add Recipients *</label>
            <input
              v-model="userSearch"
              @input="searchUsers(userSearch)"
              type="text"
              placeholder="Search by name or email..."
              class="w-full px-4 py-3 bg-gray-900 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:border-yellow-500 focus:outline-none transition text-sm">
            ...
            <div v-if="form.recipients.length > 0" class="space-y-2">
              <p class="text-sm font-semibold text-gray-300">{{ form.recipients.length }} recipient(s)</p>
              <div class="space-y-2">
                <div
                  v-for="recipient in form.recipients"
                  :key="recipient._id"
                  class="flex items-center justify-between gap-3 p-3 bg-gray-900 border border-gray-600 rounded-lg">
                  <div class="min-w-0">
                    <p class="text-sm font-semibold text-white">{{ recipient.firstName }} {{ recipient.lastName }}</p>
