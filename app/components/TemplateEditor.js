    ...
    <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-gray-900 border-2 border-gray-700 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div class="p-3 md:p-4 space-y-4 md:space-y-6">
          <!-- Name -->
          <div>
            <label class="block text-xs md:text-sm font-semibold text-gray-300 mb-2">Template Name *</label>
            <input
              v-model="form.name"
              type="text"
          ...
          <!-- Category -->
          <div>
            <label class="block text-xs md:text-sm font-semibold text-gray-300 mb-2">Category</label>
            <select
              v-model="form.category"
              class="w-full px-3 md:px-4 py-2 md:py-3 bg-gray-800 border-2 border-gray-700 rounded-lg text-white text-sm focus:border-yellow-500 focus:outline-none transition">
          ...
          <!-- Subject -->
          <div>
            <label class="block text-xs md:text-sm font-semibold text-gray-300 mb-2">Subject Line *</label>
            <input
          ...
          <!-- Body -->
          <div>
            <label class="block text-xs md:text-sm font-semibold text-gray-300 mb-2">Message Body *</label>
            <textarea
              v-model="form.body"
              rows="6"
          ...
          <!-- Variables -->
          <div class="border-t border-gray-700 pt-3 md:pt-4">
            <label class="block text-xs md:text-sm font-semibold text-gray-300 mb-3">Template Variables</label>

            <!-- Add Variable -->
            <div class="space-y-2 md:space-y-3 mb-3 md:mb-4 p-3 md:p-4 bg-gray-800/50 border border-gray-700 rounded-lg">
              <div>
                <input
                  v-model="newVariable.name"
                  type="text"
                  placeholder="Variable name (e.g., firstName)"
                  class="w-full px-2 md:px-3 py-1.5 md:py-2 bg-gray-900 border border-gray-600 rounded text-white placeholder-gray-500 focus:border-yellow-500 focus:outline-none transition text-xs md:text-sm">
              </div>
              <div>
                <input
              ...
              <button
                @click="addVariable"
                type="button"
                class="w-full px-3 md:px-4 py-2 md:py-2.5 bg-yellow-600 hover:bg-yellow-500 text-gray-900 rounded font-semibold transition text-xs md:text-sm min-h-[40px] flex items-center justify-center">
                Add Variable
              </button>
            </div>

            <!-- Variable List -->
            <div v-if="form.variables.length > 0" class="space-y-1.5 md:space-y-2">
              <p class="text-xs md:text-sm text-gray-400">{{ form.variables.length }} variable(s)</p>
              <div
                v-for="(variable, index) in form.variables"
                :key="index"
                class="flex items-center justify-between p-2 md:p-3 bg-gray-800 border border-gray-700 rounded-lg gap-2">
                <div class="flex-1 min-w-0">
                  <p class="text-xs md:text-sm font-semibold text-white">{{ variable.name }}</p>
